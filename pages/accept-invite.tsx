...
// Guard: shows an error if the invite token is absent or expired
export default function AcceptInvite() {
  ...
  useEffect(() => {
    if (router.isReady && !token) {
      setError('Invalid or missing invite link. Please contact your account admin for a new one.');
    }
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) { setError(data.message ?? 'Invite acceptance failed. The link may have expired.'); return; }
      setDone(true);
  ...
  return (
    <>
      <Head><title>Set Your Password | Divine Credit System</title></Head>
      <div className="min-h-screen bg-[#f5f7fa] flex items-center justify-center p-4">
        <div className="w-full max-w-md animate-fade-in">

          <div className="flex flex-col items-center mb-8">
            <h1 className="text-2xl font-bold text-gray-900">Divine Credit System</h1>
            <p className="text-sm text-gray-500 mt-1">Welcome — set your password to access your account</p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8">
            {done ? (
              ...
              <form onSubmit={submit} className="space-y-5">
                <div>
                  <h2 className="text-lg font-bold text-slate-800 mb-1">Create your password</h2>
                  <p className="text-sm text-slate-500">Choose a strong password to access your account online.</p>
                </div>
